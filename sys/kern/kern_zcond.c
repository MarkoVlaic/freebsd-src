...
void __zcond_disable(struct zcond* cond) {
    ...
    SLIST_FOREACH(p, &cond->ins_points, next) {
       //char* patch_instruction = p->swap_page + p->patch_address & PAGE_MASK;
        ...
        pmap_protect(kernel_pmap, page_start, page_end, VM_PROT_READ | VM_ENTER_WIRED);
        memcpy((void *)p->patch_addr, nop, NOP_SIZE);
        pmap_protect(kernel_pmap, page_start, page_end, VM_PROT_READ | VM_ENTER_WIRED | VM_PROT_EXECUTE);
    }
}
